...
export default {
	...
	data() {
		return {
			...
			pol: '',
			pols: [],
			sDate: new Date(new Date().getTime() - 1000 * 3600 * 24),
			eDate: new Date(),
			time: '',
			pDate: '',
			source: "1",
	...
	methods: {
		...
		onGetConfig(config) {
			...
			this.sources = this.config.settings.sourceOption.data;
			this.source = this.config.settings.sourceOption.default;
			this.basics = this.config.settings.basicOption.data;
			this.basic = this.config.settings.basicOption.default;
			this.imageObj = {};
			this.sourceChange();
			let sTimeStr = dateUtils.dateToStr("YYYYMMDD00", this.sDate);
			let eTimeStr = dateUtils.dateToStr("YYYYMMDD23", this.eDate);
			let timeRange = [sTimeStr, eTimeStr];
